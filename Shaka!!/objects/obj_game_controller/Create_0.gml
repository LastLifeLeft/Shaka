// Game state machine
game_state = "loading";  // loading → ready → playing → finished
...
load_chart = function(_path) {
    show_debug_message($"Loading chart: {_path}");
...
setup_game = function() {
    // Create highway
    highway = instance_create_layer(
        NOTE_HIGHWAY_WIDTH / 2,
        NOTE_HIGHWAY_HEIGHT / 2,
        "Game",
        obj_note_highway
    );
    highway.mode = game_mode;
    
    // Create rhythm engine
    rhythm_engine = instance_create_depth(0, 0, 0, obj_rhythm_engine);
    rhythm_engine.chart = chart;
    ...
    if (chart.audio_file != "") {
        ...
        if (file_exists(_audio_path)) {
            ...
            show_debug_message($"Music file not found: {_audio_path}");
            show_debug_message("Game will run without audio");
        }
    }
    
    show_debug_message("Game components initialized");
}



step_loading = function() {
    // Initialize game
    if (chart == undefined) {
        // Load default chart (or chart passed from menu)
        if (!load_chart(working_directory + "charts/demo_chart.json")) {
            return;
        }
    }
...
step_ready = function() {
    // Start on any key
    if (keyboard_check_pressed(vk_anykey) && !keyboard_check(vk_escape)) {
        start_song();
    }
}

step_playing = function() {
    if (is_paused) return;
    
    // Update song time
    if (music_voice != undefined && VinylIsPlaying(music_voice)) {
        // Use Vinyl's track position for accurate timing
        current_time_ms = VinylGetTrackPosition(music_voice) * 1000;
    } else {
        // Manual timing (no audio)
        current_time_ms = current_time - song_start_time - pause_time;
    }
